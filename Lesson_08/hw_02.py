Реализация методов верная.
Но, во-первых: у вас теперь все машины и truck и car будут иметь свойства по умолчанию от Auto те, что вы жёсткопрописали в классе Auto.
Во-вторых: здесь тоже надо прописать конструктор класса __init__, чтобы выполнить условия : Класс truck имеет дополнительный обязательный атрибут max_load и Класс car имеет дополнительный обязательный атрибут max_speed
